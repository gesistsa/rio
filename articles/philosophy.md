...
2.  **rio** uses
    ...
    for text-delimited files to automatically determine the file format
    regardless of the extension. So, a CSV that is actually
    tab-separated will still be correctly imported. It’s also crazy
    fast.
...
6.  **rio** wraps a variety of faster, more stream-lined I/O packages
    ...
    [**writexl**](https://cran.r-project.org/package=writexl) for
...
7.  **rio** stores metadata from rich file formats (SPSS, Stata, etc.)
    ...
    - `format`: a character string describing the variable storage type
    ...
    reverses that gathering process). These can be useful, especially,
    during file conversion to more easily modify attributes that are
    handled differently across file formats. As an example, the
    following idiom can be used to trim SPSS value labels to the
    ...
    converts a single variable or all variables in a data frame that
    have “labels” attributes into character vectors based on the mapping
    of values to value labels.
    ...
    helpful for converting these rich file formats into open formats
    (e.g., `export(characterize(import("file.dta")), "file.csv")`.
